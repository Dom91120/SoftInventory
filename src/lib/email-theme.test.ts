...
describe("emailButton", () => {
  ...
  it("préserve une URL de vérification réelle, esperluettes comprises", () => {
    ...
    const url = "https://softinventory.exemple.fr/verif?token=abc&callbackURL=%2Fmon-compte";
    expect(emailButton(url, "Confirmer")).toContain(
      'href="https://softinventory.exemple.fr/verif?token=abc&amp;callbackURL=%2Fmon-compte"',
    );
  });
});
...
describe("wrapEmailHtml — le lien « Portail », même primitive", () => {
  ...
  it("conserve une URL de portail légitime", () => {
    expect(wrapEmailHtml("<p>x</p>", { appUrl: "https://softinventory.exemple.fr" })).toContain(
      'href="https://softinventory.exemple.fr"',
    );
  });

  it("sans URL configurée : texte simple, aucun lien", () => {
    const html = wrapEmailHtml("<p>x</p>");
    expect(html).toContain("SoftInventory");
  });
