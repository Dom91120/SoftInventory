...
/**
 * Inactivité maximale tolérée, par rôle. Application interne au réseau de la
 * collectivité : délais plus souples que sur une app exposée, mais l'admin —
 * qui peut modifier les secrets SMTP/LDAP et les comptes — reste plus court.
 */
export const MAX_IDLE_MS: Record<Role, number> = {
  lecteur: 4 * 60 * 60 * 1000, // 4 h — consultation au fil de la journée
...
export const MAX_ABSOLUTE_MS: Record<Role, number> = {
  lecteur: 24 * 60 * 60 * 1000, // 24 h
  admin: 12 * 60 * 60 * 1000, // 12 h — couvre une journée de travail étendue
};

/** Rôle par défaut si la session n'en porte pas (le moins privilégié gagne). */
const FALLBACK_ROLE: Role = "lecteur";
