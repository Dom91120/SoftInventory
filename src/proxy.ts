...
export const config = {
  matcher: [
    ...
    //
    // EXCLU AUSSI : le téléchargement de pièces jointes. La réponse n'est pas un
    // document de l'application mais un fichier déposé par un utilisateur, que la
    // route sert au besoin en `inline` sous sa PROPRE politique (`sandbox`). La
    // CSP à nonce, calculée ici, n'a pas d'objet sur une réponse binaire et
    // écraserait cette politique — le PDF affiché retrouverait le droit
    // d'exécuter ses scripts.
    "/((?!_next/static|_next/image|favicon\\.ico|api/documents/download|.*\\.[^/]+$).*)",
    "/api/auth/:path*",
  ],
};
